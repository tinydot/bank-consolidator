...
is persisted in the `ask_ai_messages` table (the live Anthropic message array,
JSON-encoded one row per message) rather than localStorage, so it survives
reloads and rides inside the single exported DB blob — i.e. it is part of both
the local `.db` download/import and the Google Drive backup/restore.
`askAiPersistHistory` rewrites the table after every completed turn (then
`markDirty()`); `askAiLoadHistory` reloads + re-renders it on startup and after a
DB import / Drive restore (trimming any dangling mid-tool-loop turn); the
**Delete history** button (`askAiClear`) wipes both memory and the table after a
`confirm()`. **Read-only is load-bearing:** `askAiIsReadOnly`
