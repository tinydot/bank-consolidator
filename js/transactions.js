...
// a.bank_id is trailing because it is only read by the duplicate view (which
// groups by bank); everything else indexes these columns positionally.
...
// shares its (bank, date, amount) with at least one other transaction in the
// current filter set, grouped so the user can eyeball each cluster and press
// Ignore on the rows they consider duplicates. Bank is part of the key
// because a duplicate is a bank re-exporting its own row — the same date and
// amount appearing at two different banks is a coincidence, not a duplicate.
//
...
async function loadDuplicateTransactions(page = 0) {
    ...
    const groupQuery = `SELECT a.bank_id, t.date, t.amount ${TRANSACTION_FROM}${where}
        GROUP BY a.bank_id, t.date, t.amount HAVING COUNT(*) > 1`;
    ...
    const options = {
        ...
        emptyMessage: 'No transactions from the same bank share the same date and amount in this view.'
    };
    ...
    const groups = dbHelpers.queryAll(
        `${groupQuery} ORDER BY t.date DESC, t.amount ASC, a.bank_id ASC LIMIT ${groupSize} OFFSET ${page * groupSize}`,
        params
    );

    // Match on the (bank, date, amount) triple rather than a concatenated key
    // so the comparison stays typed (amounts are integer cents).
    const groupWhere = groups.map(() => '(a.bank_id = ? AND t.date = ? AND t.amount = ?)').join(' OR ');
    ...
    const result = db.exec(
        `${TRANSACTION_SELECT}${TRANSACTION_FROM}${where} AND (${groupWhere})
         ORDER BY t.date DESC, t.amount ASC, a.bank_id ASC, t.id ASC`,
...
function displayTransactions(result, totalCount = 0, page = 0, options = {}) {
    ...
    // Duplicate review: group the rows by (bank, date, amount) so each cluster
    // is rendered under one header. Counts are taken from the rows themselves —
    ...
    if (groupMode) {
        ...
        hint.innerHTML = `<strong>Duplicate review.</strong> Every transaction below shares its bank, date and amount with at least one other
                ...
                headerTr.innerHTML = `
                    <td colspan="7" style="background:#eef3f7; border-top:2px solid #c8d6e0; padding:8px 10px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; gap:10px; flex-wrap:wrap;">
                            <div><strong>${escapeHtml(bank)}</strong> • <strong>${date}</strong> • <span class="${amountClass}">${amountStr}</span>
                                <span style="color:#6c7a89;"> — ${count} transactions from the same bank with the same date &amp; amount</span></div>
